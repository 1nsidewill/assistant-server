...
class QueryMetaDB:
    ...
    def get_system(self, id: str) -> str:
        return self.run("select connect_type, connect_spec from api_system where system_id = {id} and status = 'Y'".format(id), "one")
    
    def get_api(self, id: str) -> str:
        return self.run("select system_id, api_spec from api_spec where api_id = {id} and status = 'Y'".format(id), "one")
